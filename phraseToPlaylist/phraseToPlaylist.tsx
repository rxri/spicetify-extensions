...
interface PhraseToPlaylistProps {
...
(async function phraseToPlaylist() {
	...
	const PhraseToPlaylist: React.FC<PhraseToPlaylistProps> = ({ onSubmit, loadingIndicator }) => {
		...
		return (
			<>
				...
				<div style={{ display: "flex", "flex-direction": "row", "justify-content": "space-between" }}>
					<span className="phrase-loading-indicator">{loadingIndicator}</span>
					<ButtonPrimary size="sm" onClick={handleSubmit}>
						Submit Phrase
					</ButtonPrimary>
				</div>
			</>
	...
	function displayPhraseInput() {
		...
		const handleSubmit = async (phrase: string) => {
			generatePlaylist(phrase);
		};

		ReactDOM.render(<PhraseToPlaylist onSubmit={handleSubmit} loadingIndicator="0/0" />, modalContent);
	...
	function updateLoadingIndicator(current: number, total: number) {
		...
		if (spanProgress) spanProgress.textContent = `${current}/${total}`;
	}
	...
	async function isSameSong(word: string, name: string): Promise<boolean> {
