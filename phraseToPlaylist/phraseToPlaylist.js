...
async function generatePlaylist(phraseFn) {
	let phrase = phraseFn.replace(/[^\w\s\-]|_/g, "").replace(/\s+/g, " ");
	phrase = phrase.replace(/^\ |\ $/g, "").split(" "); //replace spaces at start and end of string so those don't become their own substrings
	const songArr = [],
		songMapCache = {};
	offset = "";
	for (let i = 0; i < phrase.length; i++) {
		let spanProgress = document.querySelector("#phrase-loading-indicator"),
			progressText = document.createTextNode(`${i} / ${phrase.length}`),
			textarea = document.querySelector("#playlist-phrase-box"),
			currentText = "";
		if (!spanProgress || !progressText || !textarea || !currentText) {
			console.log("[P2P]: Error while generating playlist, aborting.");
			return;
		}
		...
		if (phrase[i] in songMap || phrase[i].toUpperCase() in songMap) {
			if (phrase[i] in songMap) songArr[i] = `spotify:track:${songMap[phrase[i]]}`;
			else songArr[i] = `spotify:track:${songMap[phrase[i].toUpperCase()]}`;
...
async function searchSong(songName) {
	...
	try {
		...
		for (let i = 0; i < songJSON.tracks.items.length; i++) {
	...
	while (!songFound && offsetCounter < 1000) {
		const fallbackSongJSON = await Spicetify.CosmosAsync.get(
			`https://api.spotify.com/v1/search?q=${songName}&type=track&limit=50&offset=${offsetCounter}`
		);
		for (let i = 0; i < fallbackSongJSON.tracks.items.length; i++) {
